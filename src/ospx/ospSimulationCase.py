...
class OspSimulationCase:
    ...
    def _write_osp_model_description_xmls(self) -> None:
        """Write the <component.name>_OspModelDescription.xml files for all components.

        Writes the <component.name>_OspModelDescription.xml files for all components defined in the system structure.
        """
        logger.info(
            f"Write OspModelDescription.xml files for OSP simulation case '{self.name}' "
            f"in case folder: {self.case_folder}"
        )
    ...
    def write_osp_system_structure_xml(self) -> None:
        ...
        logger.info(
            f"Write OspSystemStructure.xml file for OSP simulation case '{self.name}' "
            f"in case folder: {self.case_folder}"
        )
        ...
        for i, (_, component) in enumerate(self.system_structure.components.items()):
            simulator_key = f"{i:06d}_Simulator"
            simulator_properties: dict[str, dict[str, str | float | dict[str, Any] | Path]] = {
                "_attributes": {
                    "name": component.name,
            ...
            if component.variables_with_start_values:
                ...
                for ii, (_, variable) in enumerate(component.variables_with_start_values.items()):
                    if variable.start is not None and variable.data_type is None:
                        logger.error(
                            f"component {component.name}: An initial value is defined for variable {variable.name}, "
                            "but its data type is not defined.\n"
                            f"The initial value for variable {variable.name} will not be written "
                            "into OspSystemStructure.xml.\n"
                            "OspSystemStructure.xml will be potentially wrong or incomplete."
                        )
                    else:
                        initial_value_key = f"{ii:06d}_InitialValue"
    ...
    def write_system_structure_ssd(self) -> None:
    ...
    def write_statistics_dict(self) -> None:
        ...
        statistics_dict: dict[str, dict[str, Any]] = {}
    ...
    def write_watch_dict(self) -> None:
    ...
    def _resolve_lib_source_folder(self) -> None:
        ...
        if "_environment" in self.case_dict:
            if "libSource" in self.case_dict["_environment"]:
                ...
                logger.warning(
                    f"no 'libSource' element found in {self.case_dict.name}['_environment']. "
                    "Path to libSource will be set to current working directory."
                )
        else:
            logger.warning(
                f"no '_environment' section found in {self.case_dict.name}. "
                "Path to libSource hence is unknown and will be set to current working directory."
            )
        self.lib_source = self.lib_source.resolve().absolute()

    def _resolve_fmu_file(self, fmu_name: str) -> Path:
        fmu_file: Path = Path(fmu_name)
        fmu_file = fmu_file.resolve() if fmu_file.is_absolute() else (self.lib_source / fmu_file).resolve()
        return fmu_file

    def _check_all_fmus_exist(self) -> None:
        """Check whether all referenced FMUs actually exist."""
    ...
    def _resolve_all_fmus(self) -> None:
        ...
        components = self.case_dict["systemStructure"]["components"]
        for component_properties in components.values():
            fmu_file = self._resolve_fmu_file(component_properties["fmu"])
            try:
    ...
    def _set_components_step_size(self, step_size: float) -> None:
    ...
    def _inspect(self) -> None:
        """Inspect all components and all FMUs for the public variable names and units they declare.

        Inspects all components and all FMUs for the public variable names and units they declare
        in their modelDescription.xml's. Results get logged to the console.
        """
        ...
        for fmu_name, fmu in self.system_structure.fmus.items():
            ...
            default_experiment_key = find_key(fmu.model_description, "DefaultExperiment$")
            if default_experiment_key and "_attributes" in fmu.model_description[default_experiment_key]:
    ...
    def _write_plot_config_json(self) -> None:
        ...
        if "plots" in self.case_dict["postProcessing"]:
